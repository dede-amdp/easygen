...
if (comment_block.includes(code_del[0])) {
    ...
    if (comment_block[comment_block.indexOf(code_del[0]) - 1] != "\\") {
        ...
        comment_block = comment_block
            .substring(
                0,
                comment_block.lastIndexOf(start_del) + start_del.length
            )
            .trim();
        var end_name =
            start_del.length +
            comment_block.substring(start_del.length).indexOf(end_del);
        var end_all =
            end_name +
            end_del.length +
            comment_block
                .substring(end_name + end_del.length)
                .lastIndexOf(start_del);
        comment_block =
            comment_block.substring(0, end_name) +
            "\n" +
            comment_block.substring(end_name + end_del.length, end_all) +
            end_del; //escape_string around the second string
    }
}
```
